...
export default defineConfig({
  cacheDir: fileURLToPath(new URL("./tmp/coordination/pos-recovery-vite", import.meta.url)),
  test: {
    include: ["apps/web/src/pos-recovery-*.test.ts", "apps/web/src/pos-recovery-*.test.tsx", "apps/api/tests/pos-recovery-*.test.ts",
      ...
      "apps/api/tests/pos-service.test.ts", "apps/api/tests/transaction-executor.test.ts"],
    pool: "threads", maxWorkers: 1, fileParallelism: false,
    exclude: ["apps/api/tests/**/*.integration.test.ts"],
  },
});

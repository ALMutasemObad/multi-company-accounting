...
test("CloudLinux registration switching recreates immutable release roots and restores the source on failure", async () => {
  ...
  assert.match(switcher, /environment\.REGISTRATION_TOKEN_SECRET/u);
  assert.match(switcher, /environment\.REGISTRATION_AUDIT_PEPPER/u);
  assert.match(switcher, /validate_registered_environment "\$target_root" true false true/u);
...
test("CI deploys only staging from current main after all database and upgrade gates", async () => {
  ...
  assert.match(source, /validateStagingEmailSource/u);
  assert.match(source, /activeEnvironment = matches\[0\]\.env_vars \|\| \{\}/u);
  assert.match(source, /REGISTRATION_EMAIL_MODE !== "resend"/u);
  assert.match(source, /REGISTRATION_EMAIL_FROM/u);
  assert.match(source, /RESEND_API_KEY/u);
  assert.match(source, /REGISTRATION_TOKEN_SECRET/u);
  assert.match(source, /REGISTRATION_AUDIT_PEPPER/u);
  ...
  assert.match(source, /\/api\/v1\/auth\/register\/options/u);
  assert.match(source, /\/api\/v1\/auth\/password\/forgot/u);
  assert.match(source, /mcap-ci-\$GITHUB_RUN_ID-\$GITHUB_RUN_ATTEMPT@example\.test/u);
  assert.match(source, /test "\$forgot_status" = 202/u);
  assert.match(source, /response\.status !== "ACCEPTED"/u);

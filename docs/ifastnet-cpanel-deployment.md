...
الاستضافة المشتركة لا توفر systemd أو صلاحية إدارة binlogs. بعد تصنيف الهدف الحالي Staging أزيل الجدول التلقائي من `production-backup-dr.yml` والمراقب المرتبط به؛ يعملان يدويًا فقط ولا يمثلان SLO إنتاجيًا. أُبقيت معرّفات Artifacts التاريخية لحفظ الأدلة السابقة، وأُبقي مؤقتًا اسم GitHub Environment القديم `production` كنطاق للأسرار الموجودة بعد حذف نسخ Repository؛ لا يعني ذلك أن الهدف Production. لا تضع عبارة التشفير أو عنوان حساب الترحيل في Cron أو Selector؛ كلاهما يمر عبر `stdin`. انقل الأسرار لاحقًا بإعادة إدخالها من مدير الأسرار إلى Environment باسم `staging`، وأثبت تشغيلات يدوية ناجحة، ثم احذف القيم القديمة. يجب إنشاء النسخ المجدولة والمراقبة على بيئة وأسرار منفصلة قبل إطلاق Production الحقيقي. راجع [دليل التعافي](production-disaster-recovery.md).

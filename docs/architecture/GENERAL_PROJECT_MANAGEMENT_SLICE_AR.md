...
تعديل مهمة داخلها، وتمنع المهمة النهائية تعديلها أو إسنادها أو إضافة تعليق موجه
إليها. لا تغير اعتمادياتها، عدا إزالة استردادية ضيقة لرابط فعال تكون فيه السابقة
`CANCELLED` والتابعة غير نهائية. لا reopen أو cascade ضمني في الشرائح الأولى.
...
| الأمر | السابقة | التابعة | المتطلبات |
|---|---|---|---|
| إضافة | غير نهائية | غير نهائية | `general_projects.manage` و`expectedPlanVersion` |
| إزالة عادية | غير نهائية | غير نهائية | `general_projects.manage` و`expectedPlanVersion` و`expectedVersion` و`reason` |
| إزالة بعد إلغاء السابقة | `CANCELLED` فقط | `TODO/IN_PROGRESS/BLOCKED` | الشروط نفسها، والرابط فعال؛ إزالة فقط بلا تعديل أو إضافة |

الصف الثالث هو الاستثناء الوحيد لقاعدة الأطراف غير النهائية. يقفل الأمر المشروع ثم
المهمتين بترتيب المعرف ثم الاعتمادية، ويعيد فحص الشركة والحالات وفعالية الرابط، وينفذ
CAS على `expectedVersion`، ويزيد `planVersion` مرة واحدة، ويسجل
`GENERAL_PROJECT_TASK_DEPENDENCY_REMOVED_AFTER_PREDECESSOR_CANCELLED` والسبب المنقح
في Audit داخل المعاملة نفسها. لا يفتح التابعة ولا يغير `BLOCKED` اليدوية، ولا يكتب
Outbox لعدم وجود مستهلك في GPM-3.
...
`expectedPlanVersion`، وتحمل الإزالة `expectedVersion` للرابط و`reason`. لا تسمح
الإضافة بطرف نهائي. تسمح الإزالة الاستردادية فقط عندما تكون السابقة `CANCELLED`
والتابعة غير نهائية والمشروع غير نهائي، وفق المعاملة والتدقيق المحددين أعلاه.
...
- نجاح إزالة الرابط الفعال من سابقة `CANCELLED` إلى تابعة غير نهائية مرة واحدة فقط،
  وتحديث نسخة الرابط و`planVersion` وAudit ذريًا.
- رفض الاستثناء مع سابقة `COMPLETED` أو غير نهائية، أو تابعة نهائية، أو رابط غير
  فعال، أو `expectedVersion/expectedPlanVersion` قديمين؛ ورفض أي إضافة/تعديل لطرف
  نهائي.
- سباق الإزالة الاستردادية مع انتقال التابعة يحسمه القفل/النسخة: أثر واحد أو Conflict
  بلا رابط نصف محدث، ولا Outbox بلا مستهلك.
...
- الإزالة الاستردادية بعد إلغاء السابقة: replay بالمفتاح نفسه يعيد الأثر نفسه بلا
  Audit مكرر، وpayload مختلف أو نسخة قديمة يرفض، وسباق انتقال التابعة لا يوسع
  الاستثناء إلى طرف نهائي.

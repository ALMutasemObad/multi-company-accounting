...
تطبق مطالبات مصروفات الموظفين ترتيبًا محدودًا: سجل Idempotency، ثم صف الموظف،
ثم صف المطالبة، ثم مراكز التكلفة تصاعديًا، ثم التدقيق. ولا
تقفل Treasury أو الفترة أو Ledger لأن الحالة الحالية تنتهي عند
`READY_FOR_PAYMENT` بلا أثر مالي. أي عقد صرف لاحق يجب أن يضيف ترتيبه إلى هذه
السياسة قبل التنفيذ، لا أن يستدعي Treasury بعد إمساك أقفال بترتيب معاكس.
...
| طلبا POS متزامنان بالمفتاح والجسم نفسيهما | رابط وفاتورة وحركة مخزون وسند قبض وقيود مرة واحدة |

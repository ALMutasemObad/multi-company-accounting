...
## ملحق إغلاق الدين المعماري عالي المخاطر — 28 أغسطس 2026

يحفظ متن التقرير صورة 21 أغسطس ولا يعاد تفسير أدلته التاريخية. أغلقت دفعة لاحقة
الملاحظات البنيوية المتبقية ذات الأولوية العالية كما يلي:

- انتقل `ActorContext` إلى Application Kernel، وأصبح تجهيز الشركة والتسجيل وفحص
  استخدام العملات تنسيقًا عبر منافذ المالكين بدل Prisma عابر للسياقات.
- أصبح Sales مالك `Customer/CustomerAddress`، وأصبح Data Import مستهلكًا لمنفذ صغير.
- أصبح `AuditLog` و`SecurityEvent` بمالك كتابة واحد، وتمر قراءات الفاعلين عبر Identity
  Query Ports. وانتقلت قراءات الحسابات والطباعة إلى Query/Locator Adapters صريحة.
- أضيف مخزن Rate Limit مشترك للمسارات العامة الحساسة مع فشل مغلق وبصمة HMAC سرية،
  وحدود مستقلة للشبكة والجلسة/البريد، وأصبحت ذاكرة نوافذ التنبيه محدودة بالزمن لا بعدد الأحداث.
- أصبحت مهلة Outbox Handler نافذة حتى عند تجاهل `AbortSignal`، وأزيل `any` من خدمات
  الأوامر المالية الأساسية.
- تثبت هذه الحدود اختبارات Architecture Guardrails. تبقى بوابتا MariaDB 10.11 وMySQL
  8.4 الفعليتان، بما فيهما ترقية baseline المحمل بالبيانات، شرط نشر لا يمكن إثباته
  في بيئة محلية بلا Docker/WSL.

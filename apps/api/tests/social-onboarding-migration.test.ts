...
describe('social onboarding migration', () => {
  it('keeps the social onboarding migration and stores only hashed or protected continuation material', async () => {
    ...
    expect(directories.filter((entry) => entry.isDirectory()).map((entry) => entry.name))
      .toContain('20260906210000_social_onboarding_continuation');

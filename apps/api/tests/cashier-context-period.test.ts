...
describe("Core Accounting cashier context period advisory (unit, no DB)", () => {
  ...
  it("keeps the actual initial version zero instead of fabricating a later revision", () => {
    expect(classifyCashierContextPeriods("2026-08-31", [row({ version: 0 })])).toMatchObject({
      status: "RESOLVED", period: { id: "81", version: 0 },
    });
  });

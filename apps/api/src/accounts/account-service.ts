...
export type AccountErrorReason = 'NOT_FOUND' | 'CODE_EXISTS' | 'INVALID_PARENT' | 'CYCLE_DETECTED' | 'LEVEL_EXCEEDED' | 'HAS_ACTIVE_CHILDREN' | 'POSTING_NOT_ALLOWED';
...
type AccountUpdate = { accountTypeId?: bigint | undefined; parentAccountId?: bigint | null | undefined; nameAr?: string | undefined; nameEn?: string | null | undefined; allowsPosting?: boolean | undefined; isControlAccount?: boolean | undefined };
type CostCenterInput = { parentId?: bigint | null | undefined; code: string; nameAr: string; nameEn?: string | null | undefined };
...
export class AccountService {
  ...
  async createCostCenter(context: ActorContext, input: CostCenterInput) { try { return await this.prisma.$transaction(async (tx) => { if (input.parentId != null && !await tx.costCenter.findFirst({ where: { id: input.parentId, companyId: context.companyId, isActive: true } })) throw new AccountError('INVALID_PARENT'); const value = await tx.costCenter.create({ data: { companyId: context.companyId, parentId: input.parentId ?? null, code: input.code, nameAr: input.nameAr, nameEn: input.nameEn ?? null } }); await this.audit(tx, context, 'COST_CENTER_CREATED', 'COST_CENTER', value.id); return value; }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); } catch (error) { if (knownUnique(error)) throw new AccountError('CODE_EXISTS'); throw error; } }

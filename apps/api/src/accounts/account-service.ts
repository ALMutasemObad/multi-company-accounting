...
export class AccountService {
  ...
  async updateAccount(context: ActorContext, id: bigint, input: AccountUpdate) {
    ...
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); } catch (error) { if (knownUnique(error)) throw new AccountError('CODE_EXISTS'); if (knownWriteConflict(error)) throw new AccountError('VERSION_CONFLICT'); throw error; }
  }

  async deactivateAccount(context: ActorContext, id: bigint, reason: string, expectedVersion: number) {
    try { return await this.prisma.$transaction(async (tx) => { const current = await tx.account.findFirst({ where: { id, companyId: context.companyId } }); if (!current) throw new AccountError('NOT_FOUND'); if (current.version !== expectedVersion) throw new AccountError('VERSION_CONFLICT'); if (await tx.account.count({ where: { companyId: context.companyId, parentAccountId: id, isActive: true } })) throw new AccountError('HAS_ACTIVE_CHILDREN'); const changed = await tx.account.updateMany({ where: { id, companyId: context.companyId, version: expectedVersion }, data: { isActive: false, version: { increment: 1 } } }); if (changed.count !== 1) throw new AccountError('VERSION_CONFLICT'); const value = await tx.account.findFirstOrThrow({ where: { id, companyId: context.companyId }, include: { accountType: true } }); await this.audit(tx, context, 'ACCOUNT_DEACTIVATED', 'ACCOUNT', id, reason); return value; }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); } catch (error) { if (knownWriteConflict(error)) throw new AccountError('VERSION_CONFLICT'); throw error; }
  }
  ...
  private async descendants(tx: Prisma.TransactionClient, companyId: bigint, root: bigint) { const result: Array<{ id: bigint; level: number; version: number }> = []; let ids = [root]; while (ids.length) { const rows = await tx.account.findMany({ where: { companyId, parentAccountId: { in: ids } }, select: { id: true, level: true, version: true } }); result.push(...rows); ids = rows.map((row) => row.id); } return result; }

...
export function createAccountRouter(auth: AuthService, service: AccountService) {
  ...
  router.post('/accounts/:accountId/deactivate', async (req, res) => { const context = await authorize(req, 'accounts.deactivate', true); const body = bodies.deactivateAccount.parse(req.body); res.json(accountJson(await service.deactivateAccount(context, id.parse(req.params.accountId), body.reason, body.expectedVersion))); });
  router.delete('/accounts/:accountId', async (req, res) => { const context = await authorize(req, 'accounts.delete', true); const body = bodies.deleteAccount.parse(req.body); res.json(await service.deleteAccount(context, id.parse(req.params.accountId), body.reason, body.expectedVersion)); });

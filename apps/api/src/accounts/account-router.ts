...
const updateAccount = z.object({ accountTypeId: id.optional(), parentAccountId: nullableId.optional(), nameAr: z.string().trim().min(1).max(180).optional(), nameEn: z.string().trim().min(1).max(180).nullable().optional(), allowsPosting: z.boolean().optional(), isControlAccount: z.boolean().optional() }).strict().refine((v) => Object.keys(v).length > 0);
const createCostCenter = z.object({ parentId: nullableId.optional(), code: z.string().trim().min(1).max(40), nameAr: z.string().trim().min(1).max(160), nameEn: z.string().trim().min(1).max(160).nullable().optional() }).strict();
...
const accountJson = (v: any) => ({ id: v.id.toString(), accountTypeId: v.accountTypeId.toString(), parentAccountId: v.parentAccountId?.toString() ?? null, code: v.code, nameAr: v.nameAr, nameEn: v.nameEn, level: v.level, allowsPosting: v.allowsPosting, isControlAccount: v.isControlAccount, isActive: v.isActive });

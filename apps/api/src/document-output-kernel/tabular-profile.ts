...
export function tableToCsv(rows: TabularRows) {
...
function xlsxCell(cell: TabularCell, reference: string) {
  const style = cell.style ?? 0;
  if (cell.numeric && cell.value !== "" && isCanonicalDecimal(cell.value)) return `<c r="${reference}" s="${style}" t="n"><v>${cell.value}</v></c>`;
  return `<c r="${reference}" s="${style}" t="inlineStr"><is><t xml:space="preserve">${xml(cell.value)}</t></is></c>`;
}

function tabularColumnWidths(columnCount: number) {
  if (columnCount === 8) return [16, 24, 16, 32, 14, 14, 22, 22];
  if (columnCount === 7) return [14, 20, 42, 16, 16, 18, 18];
  if (columnCount === 2) return [72, 20];
  return [42, ...Array.from({ length: columnCount - 1 }, () => 18)];
}
...
function zipStore(files: Array<{ name: string; data: Buffer }>) {
  ...
  for (const file of files) {
    ...
    directory.writeUInt32LE(crc, 16); directory.writeUInt32LE(file.data.length, 20); directory.writeUInt32LE(file.data.length, 24); directory.writeUInt16LE(name.length, 28); directory.writeUInt32LE(offset, 42);

...
export class GroupCompanyOnboardingService {
  ...
  constructor(prisma: PrismaClient, private readonly ports: {
    ...
    accounting: AccountingCompanyProvisioningPort; treasury: TreasuryCompanyProvisioningPort;
  ...
  async options(userId: bigint, organizationId: bigint) {
    ...
    return { currencies: await this.ports.tenant.currencies(), timezones: [...new Set(["UTC", ...Intl.supportedValuesOf("timeZone")])] };
  }

  create(userId: bigint, organizationId: bigint, key: string, input: GroupCompanyInput): Promise<GroupCompanyResult> {
    if (!input.companyName.trim() || input.companyName.length > 200 || !/^[A-Z]{3}$/.test(input.baseCurrencyCode)) throw new GroupCompanyOnboardingError("INVALID_COMPANY_OPTION");
    ...
    const normalized = { companyName: input.companyName.trim(), timezone: input.timezone, baseCurrencyCode: input.baseCurrencyCode };
    return this.commands.execute({
      ...
      await this.ports.accounting.provisionAccounting(tx, company.id, true);

...
export async function archiveDocument(tx: Prisma.TransactionClient, context: ActorContext, documentId: bigint) {
  ...
  if (!["RECEIPT", "PAYMENT", "MANUAL_JOURNAL", "PURCHASE_INVOICE", "PURCHASE_DEBIT_NOTE", "SALES_INVOICE", "SALES_CREDIT_NOTE"].includes(document.documentType)) throw new Error("DOCUMENT_NOT_PRINTABLE");
  ...
  const snapshot: PrintSnapshot = {
    ...
    invoice: document.purchaseInvoice ? {
      partyKind: "SUPPLIER",
      partyName: document.purchaseInvoice.supplierNameSnapshot,
      ...
      partyKind: "CUSTOMER",
      partyName: document.salesInvoice.customerNameSnapshot,
      partyTaxMasked: document.salesInvoice.customerTaxLast4 ? `****${document.salesInvoice.customerTaxLast4}` : null,
      partyAddress: document.salesInvoice.customerAddressSnapshot,
      externalInvoiceNumber: null,
      sourceInvoiceNumber: document.salesInvoice.sourceInvoice?.accountingDocument.documentNumber ?? null,
      warehouseCode: document.salesInvoice.warehouseCodeSnapshot,
      warehouseName: document.salesInvoice.warehouseNameSnapshot,
      dueDate: dateOnly(document.salesInvoice.dueDate),
      currencyCode: document.salesInvoice.currency.code,
      exchangeRate: document.salesInvoice.exchangeRate.toFixed(8),
      subtotal: document.salesInvoice.subtotal.toFixed(4),
      discountTotal: document.salesInvoice.discountTotal.toFixed(4),
      taxTotal: document.salesInvoice.taxTotal.toFixed(4),
      total: document.salesInvoice.total.toFixed(4),
      baseTotal: document.salesInvoice.baseTotal.toFixed(4),
      notes: document.salesInvoice.notes,

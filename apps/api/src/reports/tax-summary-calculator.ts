...
export function createTaxSummaryAccumulator(source: TaxSummarySourceHeader, query: TaxSummaryQuery) {
  ...
  return {
    add(invoices: TaxSummarySourceInvoice[]) {
...
export function calculateTaxSummary(source: TaxSummarySourceData, query: TaxSummaryQuery) {
  const accumulator = createTaxSummaryAccumulator(source, query);
  accumulator.add(source.invoices);

...
function hasRetiredDatabaseBrand(databaseUrl: string) {
  try {
    return retiredDatabaseBrand.test(decodeURIComponent(new URL(databaseUrl).pathname));
  } catch {
    return false;
  }
}

const unsignedBigIntCsv = z.string().trim()
  .regex(/^(?:[1-9][0-9]*(?:\s*,\s*[1-9][0-9]*)*)?$/u)
  .refine(
    (value) => {
      try {
        return !value || value.split(',').every((item) => BigInt(item.trim()) <= 18_446_744_073_709_551_615n);
      } catch {
        return false;
      }
    },
    'Every platform operator user ID must fit an unsigned BIGINT',
  );
...
const configSchema = z.object({
  ...
  DATABASE_POOL_CONNECTION_LIMIT: z.coerce.number().int().min(1).max(100).default(10),
  DATABASE_POOL_MIN_IDLE: z.coerce.number().int().min(1).max(100).default(1),
  DATABASE_POOL_ACQUIRE_TIMEOUT_MS: z.coerce.number().int().min(250).max(120_000).default(10_000),
  DATABASE_CONNECT_TIMEOUT_MS: z.coerce.number().int().min(250).max(30_000).default(1_000),
  ...
  SOCIAL_ONBOARDING_CONTINUATION_TTL_MINUTES: z.coerce.number().int().min(2).max(15).default(10),
  ...
  PLATFORM_OPERATOR_USER_IDS: unsignedBigIntCsv.default(''),
  PLATFORM_OPERATOR_EMAILS: z.string().trim().regex(/^(?:[^,\s]+@[^,\s]+(?:\s*,\s*[^,\s]+@[^,\s]+)*)?$/u).default(''),
  PLATFORM_PAYMENT_PROVIDER_MODE: z.enum(['disabled', 'development']).default('disabled'),
  PLATFORM_PAYMENT_DEVELOPMENT_WEBHOOK_SECRET: z.string().min(32).max(500).optional(),
  PLATFORM_PAYMENT_WEBHOOK_TOLERANCE_SECONDS: z.coerce.number().int().min(30).max(900).default(300),
  ...
  if (config.DATABASE_POOL_MIN_IDLE > config.DATABASE_POOL_CONNECTION_LIMIT) {
    context.addIssue({ code: 'custom', path: ['DATABASE_POOL_MIN_IDLE'], message: 'DATABASE_POOL_MIN_IDLE must not exceed DATABASE_POOL_CONNECTION_LIMIT' });
  }
  if (config.DATABASE_CONNECT_TIMEOUT_MS > config.DATABASE_POOL_ACQUIRE_TIMEOUT_MS) {
    context.addIssue({ code: 'custom', path: ['DATABASE_CONNECT_TIMEOUT_MS'], message: 'DATABASE_CONNECT_TIMEOUT_MS must not exceed DATABASE_POOL_ACQUIRE_TIMEOUT_MS' });
  }
  if (config.PLATFORM_PAYMENT_PROVIDER_MODE === 'development' && !config.PLATFORM_PAYMENT_DEVELOPMENT_WEBHOOK_SECRET) {
    context.addIssue({
      code: 'custom',
      path: ['PLATFORM_PAYMENT_DEVELOPMENT_WEBHOOK_SECRET'],
      message: 'The development payment adapter requires an explicit webhook signing secret',
    });
  }
  if (config.NODE_ENV === 'production' && config.PLATFORM_PAYMENT_PROVIDER_MODE !== 'disabled') {
    context.addIssue({
      code: 'custom',
      path: ['PLATFORM_PAYMENT_PROVIDER_MODE'],
      message: 'The development payment adapter is forbidden in production; a reviewed commercial adapter is not included',
    });
  }
  if (config.NODE_ENV === 'production' && config.PLATFORM_PAYMENT_DEVELOPMENT_WEBHOOK_SECRET) {
    context.addIssue({
      code: 'custom',
      path: ['PLATFORM_PAYMENT_DEVELOPMENT_WEBHOOK_SECRET'],
      message: 'The development payment webhook secret is forbidden in production',
    });
  }
  ...
  } else if (hasRetiredDatabaseBrand(config.DATABASE_URL)) {
    context.addIssue({ code: 'custom', path: ['DATABASE_URL'], message: 'DATABASE_URL schema name must use the neutral identifier before deployment' });
  }
  ...
  if (config.PLATFORM_OPERATOR_EMAILS) {
    context.addIssue({ code: 'custom', path: ['PLATFORM_OPERATOR_EMAILS'], message: 'PLATFORM_OPERATOR_EMAILS is development-only; production platform access must use PLATFORM_OPERATOR_USER_IDS' });
  }
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env): LoadedAppConfig {
  ...
  return {
    ...
    DATABASE_URL: databaseUrlWithPoolOptions(config.DATABASE_URL, {
      ...
      idleTimeoutSeconds: config.DATABASE_POOL_IDLE_TIMEOUT_SECONDS,
    }),
  };
}

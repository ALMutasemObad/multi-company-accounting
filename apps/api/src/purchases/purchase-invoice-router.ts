...
const fields = {
  documentType: z.enum(["PURCHASE_INVOICE", "PURCHASE_DEBIT_NOTE"]),
  fiscalPeriodId: id,
  ...
  currencyId: id,
  exchangeRate,
  supplierAddress: z.string().trim().max(500).nullable().optional(),
...
const taxInput = z.object({ code: z.string().trim().min(1).max(40), nameAr: z.string().trim().min(1).max(120), rate, inputTaxAccountId: nullableId.optional() }).strict();
...
export function createPurchaseInvoiceRouter(auth: AuthService, service: PurchaseInvoiceService) {
  ...
  router.post("/purchase-invoices", async (request, response) => {
    ...
    response.status(201).json(PurchaseInvoiceService.json(await service.create(context, create.parse(request.body) as any)));
  });
  ...
  router.patch("/purchase-invoices/:invoiceId", async (request, response) => {
    ...
    response.json(PurchaseInvoiceService.json(await service.update(context, id.parse(request.params.invoiceId), update.parse(request.body) as any)));
  });
  ...
  router.get("/purchase-tax-rates", async (request, response) => {
    const context = await authorize(request, "purchase_invoices.view", false);
    const activeOnly = z.enum(["true", "false"]).transform((value) => value === "true").optional().parse(request.query.activeOnly);
    response.json({ data: (await service.listTaxRates(context, activeOnly)).map(PurchaseInvoiceService.taxRateJson) });
  });

  router.post("/purchase-tax-rates", async (request, response) => {
    const context = await authorize(request, "input_tax_rates.manage", true);
    response.status(201).json(PurchaseInvoiceService.taxRateJson(await service.createTaxRate(context, taxInput.parse(request.body) as any)));
  });

  router.patch("/purchase-tax-rates/:taxRateId", async (request, response) => {
    const context = await authorize(request, "input_tax_rates.manage", true);
    response.json(PurchaseInvoiceService.taxRateJson(await service.updateTaxRate(context, id.parse(request.params.taxRateId), taxUpdate.parse(request.body) as any)));
  });

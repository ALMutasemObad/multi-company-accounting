...
function AdjustmentForm({ position, eventType, onClose, onSaved }: { position: Position; eventType: "INCREASE" | "DECREASE"; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setSaving(true); setError(""); try { await api("/external-stock-positions/events", { method: "POST", idempotencyKey: idempotencyKey("external-stock", crypto.randomUUID()), body: JSON.stringify({ positionId: position.id, eventType, quantity, inventoryValueBase: valued ? value : null, sourceReference: reference, effectiveDate: date }) }); await onSaved(); } catch (cause) { setError(cause instanceof Error ? cause.message : copy.updateError); } finally { setSaving(false); } }

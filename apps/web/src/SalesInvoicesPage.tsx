...
export function SalesInvoicesPage({ notify }: { notify: Notice }) {
  ...
  const availableSections = availableDocumentTabs(allows(permissionSet, endpointPermissionPolicies.receivablesAging));
  ...
  function moveSection(event: React.KeyboardEvent<HTMLButtonElement>, current: Section) {
    ...
    selectSection(next);
    requestAnimationFrame(() => document.getElementById(documentTabId("sales", next))?.focus());
  }
  ...
  return <section className="workspace-page sales-workspace">
    <PageHeader kicker={t("pages.sales-invoices.012")} title={t("pages.sales-invoices.013")} description={t("pages.sales-invoices.014")} actions={activeSection === "invoices" && <Can policy={permissions.create}><div className="page-actions"><Button variant="secondary" icon="reverse" onClick={() => setForm({ type: "SALES_CREDIT_NOTE", invoice: null })}>{t("pages.sales-invoices.015")}</Button><Button icon="plus" onClick={() => setForm({ type: "SALES_INVOICE", invoice: null })}>{t("pages.purchase-invoices.016")}</Button></div></Can>} />
    <div className="section-tabs sales-tabs" role="tablist" aria-label={t("pages.sales-invoices.013")}>
      {availableSections.map((item) => <button key={item} type="button" id={documentTabId("sales", item)} role="tab" aria-selected={activeSection === item} aria-controls={documentPanelId("sales")} tabIndex={activeSection === item ? 0 : -1} className={activeSection === item ? "active" : ""} onClick={() => selectSection(item)} onKeyDown={(event) => moveSection(event, item)}>{t(item === "invoices" ? "pages.purchase-invoices.017" : item === "aging" ? "pages.purchase-invoices.018" : "pages.purchase-invoices.019")}</button>)}
    </div>
    <div id={documentPanelId("sales")} role="tabpanel" aria-labelledby={documentTabId("sales", activeSection)}>

...
function InvoiceForm({ type, invoice, references, onClose, onSaved }: { type: InvoiceType; invoice: SalesInvoice | null; references: References; onClose: () => void; onSaved: (value: SalesInvoice) => void }) {
  ...
  return <Modal title={invoice ? t("pages.payments.046", { value1: invoice.document.documentNumber }) : type === "SALES_INVOICE" ? t("pages.sales-invoices.056") : t("pages.sales-invoices.057")} description={t("pages.purchase-invoices.059")} onClose={onClose} wide><form className="form-grid sales-invoice-form" onSubmit={submit}>{errors.length > 0 && <div className="form-error full" role="alert">{errors.map((error) => <p key={error}>{error}</p>)}</div>}
    ...
    <fieldset className="full invoice-lines-field"><legend>{t("pages.purchase-invoices.077")}</legend><div className="invoice-line-list">{lines.map((line, index) => <div className="invoice-line-editor" key={index}><span className="line-index">{index + 1}</span><label className="line-description"><span>{t("pages.purchase-invoices.078")}</span><input value={line.description} onChange={(event) => setLine(index, { description: event.target.value })} required /></label><label className="line-quantity"><span>{t("pages.purchase-invoices.079")}</span><input dir="ltr" inputMode="decimal" value={line.quantity} onChange={(event) => setLine(index, { quantity: event.target.value })} required /></label><label className="line-unit-price"><span>{t("pages.purchase-invoices.080")}</span><input dir="ltr" inputMode="decimal" value={line.unitPrice} onChange={(event) => setLine(index, { unitPrice: event.target.value })} required /></label><label className="line-discount"><span>{t("pages.purchase-invoices.081")}</span><input dir="ltr" inputMode="decimal" value={line.discountAmount} onChange={(event) => setLine(index, { discountAmount: event.target.value })} required /></label><label className="line-account"><span>{t("pages.sales-invoices.080")}</span><select value={line.revenueAccountId} onChange={(event) => setLine(index, { revenueAccountId: event.target.value })} required><option value="">{t("pages.purchase-invoices.083")}</option>{revenueAccounts.map((account) => <option key={account.id} value={account.id}>{account.code} — {localizedReferenceName(account)}</option>)}</select></label><label className="line-tax"><span>{t("pages.purchase-invoices.084")}</span><select value={line.taxRateId} onChange={(event) => setLine(index, { taxRateId: event.target.value })}><option value="">{t("pages.purchase-invoices.085")}</option>{references.taxRates.filter((tax) => tax.isActive).map((tax) => <option key={tax.id} value={tax.id}>{localizedReferenceName(tax)} ({Number(tax.rate)}%)</option>)}</select></label><label className="line-cost-center"><span>{t("pages.manual-journals.057")}</span><select value={line.costCenterId} onChange={(event) => setLine(index, { costCenterId: event.target.value })}><option value="">{t("pages.purchase-invoices.087")}</option>{references.costCenters.map((center) => <option key={center.id} value={center.id}>{center.code} — {localizedReferenceName(center)}</option>)}</select></label><Button type="button" className="line-delete" variant="ghost" icon="trash" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((_, position) => position !== index))}>{t("pages.accounts.050")}</Button></div>)}</div><Button type="button" variant="secondary" icon="plus" onClick={() => setLines((current) => [...current, blankLine()])}>{t("pages.purchase-invoices.089")}</Button></fieldset>

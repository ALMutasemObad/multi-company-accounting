...
export function createCashierContextController(reader: CashierContextReadPort, now: () => number = () => performance.now(), reviewMaxAgeMs = 300_000) {
  ...
  function ready(forPendingCheckout = false) {
    return (canEdit() || (forPendingCheckout && lock === "checkout-pending" && canReviewCashierContext(scope)))
      && period.status === "RESOLVED" && period.documentDate === documentDate
      ...
      if (result.status === "available" && safeReference(result.reference, choice.id)
        ...
        setField(field, { ...choice, source: Object.hasOwn(draft, field) ? "draft" : choice.source, status: "available",
          reference: { id: value.id, label: value.label, revision: value.revision,
            ...
            ...(field === "currencyId" && typeof value.isBase === "boolean" ? { isBase: value.isBase } : {}),
            ...(typeof value.code === "string" ? { code: value.code } : {}),
            ...(typeof value.nameAr === "string" ? { nameAr: value.nameAr } : {}),
  ...
  return {
    ...
    setLock(next: CashierContextLock) { if (next !== lock) { lock = next; publish(next !== "checkout-pending"); } },
    saveDraft() {
      if (!canEdit()) return false;
    ...
    /** Read-only final dispatch check; it cannot approve, edit, unlock or extend freshness. */
    getReviewed: (options?: { forPendingCheckout: true }) => ready(options?.forPendingCheckout) && reviewed && fresh(reviewedAt) ? { ...reviewed } : null,

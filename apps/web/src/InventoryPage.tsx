...
type Tab = InventorySection | "valuation-report";
...
export function InventoryPage({ notify, section, onSectionChange }: {
  ...
  const tabs: Tab[] = [...visibleInventorySections(permissionSet), ...(permissionSet.has("inventory_movements.view") ? ["valuation-report" as const] : [])];
  ...
  return <section className="workspace-page">
    ...
    <div className="section-tabs" role="tablist" aria-label={t("inventory.tabs.label")}>
      {tabs.map((value) => <button key={value} type="button" role="tab" aria-selected={tab === value} className={tab === value ? "active" : ""} onClick={() => { setTab(value); if (value !== "valuation-report") onSectionChange?.(value); }}>{value === "valuation-report" ? reportCopy.tab : t(`inventory.tabs.${value}`)}</button>)}
    </div>
...
function ItemForm({ item, units, onClose, onSaved }: { item: InventoryItem | null; units: UnitOfMeasure[]; onClose: () => void; onSaved: () => void }) {
  ...
  const expectedImageVersion = item?.image?.version ?? null;
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (removeImage && !pendingImage && expectedImageVersion) await api(`/inventory-items/${saved.id}/image`, { method: "DELETE", headers: { "If-Match": `"product-image-v${expectedImageVersion}"` } });
      onSaved();
    } catch (cause) {
      setError(catalogSaved ? t("inventory.items.imagePartialSaveError") : cause instanceof Error ? cause.message : t("inventory.items.saveError"));
    } finally {
      setSaving(false);
    }
  }
...
function ProductThumbnail({ src, alt }: { src?: string | null; alt: string }) {
  ...
  return <span className="product-thumbnail">{src && !failed ? <img src={src} alt={alt} width={48} height={48} loading="lazy" decoding="async" onError={() => setFailed(true)} /> : <span aria-hidden="true" />}</span>;
}

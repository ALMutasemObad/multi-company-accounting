...
export function PurchaseInvoicesPage({ notify }: { notify: Notice }) {
  ...
  const loadReferences = useCallback(async () => {
    try {
      const [periods, currencies] = await Promise.all([
        api<ListResponse<FiscalPeriod>>("/fiscal-periods?page=1&pageSize=100"),
        api<{ data: Currency[] }>("/currencies"),
      ]);
      setReferences({ periods: periods.data.filter((item) => item.status !== "CLOSED"), currencies: currencies.data });
...
type DraftLine = { inventoryItemId: string; inventoryItemLabel: string; description: string; quantity: string; unitPrice: string; discountAmount: string; debitAccountId: string; debitAccountLabel: string; costCenterId: string; costCenterLabel: string; taxRateId: string; taxRateLabel: string; taxRateRate: string };
...
function InvoiceForm({ type, invoice, references, onClose, onSaved }: { type: InvoiceType; invoice: PurchaseInvoice | null; references: References; onClose: () => void; onSaved: (value: PurchaseInvoice) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!allows(permissionSet, invoice ? actionPermissionPolicies.purchaseInvoices.update : actionPermissionPolicies.purchaseInvoices.create)) return;
...
function lineDraft(line: PurchaseInvoiceLine): DraftLine { return { inventoryItemId: line.inventoryItemId ?? "", inventoryItemLabel: line.inventoryItemId ? `${line.inventoryItemCodeSnapshot} — ${line.inventoryItemNameSnapshot} (${line.unitOfMeasureCodeSnapshot})` : "", description: line.description, quantity: line.quantity, unitPrice: line.unitPrice, discountAmount: line.discountAmount, debitAccountId: line.debitAccountId, debitAccountLabel: line.debitAccount ? `${line.debitAccount.code} — ${line.debitAccount.nameAr}` : "", costCenterId: line.costCenterId ?? "", costCenterLabel: line.costCenter ? `${line.costCenter.code} — ${line.costCenter.nameAr}` : "", taxRateId: line.taxRateId ?? "", taxRateLabel: line.taxRate ? `${localizedReferenceName(line.taxRate)} (${Number(line.taxRate.rate)}%)` : "", taxRateRate: line.taxRate?.rate ?? line.taxRateSnapshot ?? "0" }; }
...
function TaxRatesPanel({ notify }: { notify: Notice }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(""); try { const query = new URLSearchParams({ page: String(page), pageSize: "10", activeOnly: "false", ...(submittedSearch ? { search: submittedSearch } : {}) }); const result = await api<ListResponse<TaxRate>>(`/purchase-tax-rates?${query}`); setRows(result.data); setMeta(result.meta); } catch (cause) { setError(cause instanceof Error ? cause.message : t("referencePicker.loadError")); } finally { setLoading(false); } }, [page, submittedSearch]);
  useEffect(() => { void load(); }, [load]);
  return <article className="panel tax-rates-panel"><header><div><h2>{t("pages.purchase-invoices.124")}</h2><p>{t("pages.purchase-invoices.125")}</p></div><Can policy={managePolicy}><Button icon="plus" onClick={() => setEditing(null)}>{t("pages.purchase-invoices.126")}</Button></Can></header><form className="search-box" onSubmit={(event) => { event.preventDefault(); setPage(1); setSubmittedSearch(search.trim()); }}><input aria-label={t("pages.accounts.025")} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("pages.accounts.025")} /><button type="submit">{t("pages.accounts.026")}</button></form>{error ? <div className="inline-notice" role="alert">{error}</div> : loading ? <Spinner label={t("common.loading")} /> : !rows.length ? <EmptyState title={t("pages.purchase-invoices.127")} description={t("pages.purchase-invoices.128")} /> : <><div className="data-table-wrap flat" role="region" tabIndex={0} aria-label={t("common.scrollableTable")}><table className="data-table"><thead><tr><th>{t("pages.accounts.059")}</th><th>{t("pages.fiscal.054")}</th><th>{t("pages.purchase-invoices.131")}</th><th>{t("pages.purchase-invoices.132")}</th><th>{t("pages.accounts.043")}</th><th></th></tr></thead><tbody>{rows.map((tax) => <tr key={tax.id}><td><span className="code-pill">{tax.code}</span></td><td>{localizedReferenceName(tax)}</td><td>{Number(tax.rate)}%</td><td>{tax.inputTaxAccount ? `${tax.inputTaxAccount.code} — ${tax.inputTaxAccount.nameAr}` : t("pages.purchase-invoices.133")}</td><td><span className={`status-chip ${tax.isActive ? "active" : "inactive"}`}>{tax.isActive ? t("pages.admin.065") : t("pages.purchase-invoices.135")}</span></td><td><Can policy={managePolicy}><Button variant="ghost" icon="edit" onClick={() => setEditing(tax)}>{t("pages.accounts.048")}</Button></Can></td></tr>)}</tbody></table></div><Pagination {...meta} page={page} onChange={setPage} /></>}{editing !== undefined && allows(permissionSet, managePolicy) && <TaxRateForm taxRate={editing} onClose={() => setEditing(undefined)} onSaved={async () => { setEditing(undefined); notify(t("pages.purchase-invoices.136")); await load(); }} />}</article>;
}

function TaxRateForm({ taxRate, onClose, onSaved }: { taxRate: TaxRate | null; onClose: () => void; onSaved: () => void }) {
  const { permissionSet } = useAuthorization();
  const [rate, setRate] = useState(taxRate?.rate ?? "15.0000"); const [accountId, setAccountId] = useState(taxRate?.inputTaxAccountId ?? ""); const [active, setActive] = useState(taxRate?.isActive ?? true); const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!allows(permissionSet, actionPermissionPolicies.purchaseTaxRates.manage)) return; setSaving(true); setError(""); const data = new FormData(event.currentTarget); try { await api(taxRate ? `/purchase-tax-rates/${taxRate.id}` : "/purchase-tax-rates", { method: taxRate ? "PATCH" : "POST", body: JSON.stringify({ nameAr: String(data.get("nameAr") ?? "").trim(), rate: toMoney(rate), inputTaxAccountId: Number(rate) > 0 ? accountId || null : null, ...(taxRate ? { version: taxRate.version, isActive: active } : {}) }) }); onSaved(); } catch (cause) { setError(cause instanceof Error ? cause.message : t("pages.purchase-invoices.137")); } finally { setSaving(false); } }
  return <Modal title={taxRate ? t("pages.purchase-invoices.138") : t("pages.purchase-invoices.139")} description={t("pages.purchase-invoices.140")} onClose={onClose}><form className="form-grid" onSubmit={submit}>{error && <div className="form-error full" role="alert">{error}</div>}{taxRate ? <label><span>{t("pages.purchase-invoices.141")}</span><input dir="ltr" value={taxRate.code} readOnly /></label> : <div className="inline-notice neutral full">{t("common.autoGeneratedCode")}</div>}<label><span>{t("pages.customers.046")}</span><input name="nameAr" defaultValue={taxRate?.nameAr} required /></label><label><span>{t("pages.purchase-invoices.143")}</span><input dir="ltr" inputMode="decimal" value={rate} onChange={(event) => setRate(event.target.value)} required /></label><label><span>{t("pages.purchase-invoices.144")}{Number(rate) > 0 && "*"}</span><ReferenceCombobox<Account> endpoint="/accounts?active=true&allowsPosting=true&accountClasses=ASSET" value={accountId} selectedLabel={taxRate?.inputTaxAccount ? `${taxRate.inputTaxAccount.code} — ${taxRate.inputTaxAccount.nameAr}` : ""} onChange={(account) => setAccountId(account?.id ?? "")} optionLabel={(account) => `${account.code} — ${localizedReferenceName(account)}`} placeholder={t("pages.customers.044")} searchLabel={t("pages.purchase-invoices.144")} optionalLabel={Number(rate) === 0 ? t("pages.customers.044") : undefined} required={Number(rate) > 0} /></label>{taxRate && <label className="check-field full"><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)} /><span>{t("pages.purchase-invoices.146")}</span></label>}<div className="form-actions full"><Button type="button" variant="secondary" onClick={onClose}>{t("pages.accounts.065")}</Button><Button type="submit" disabled={saving}>{saving ? t("pages.purchase-invoices.095") : t("pages.accounts.067")}</Button></div></form></Modal>;
}

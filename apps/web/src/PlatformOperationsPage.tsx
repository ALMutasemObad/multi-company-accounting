...
function BillingTab({ revision, onOpen }: { revision: number; onOpen: (target: CompanyTarget) => void }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try { setSummary(await api<PlatformBillingSummary>("/platform/billing/summary")); }
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [companyDetails, companyBilling] = await Promise.all([
        ...
        api<PlatformCompanyBilling>(`/platform/companies/${company.id}/billing`),
      ]);
...
function InvoiceForm({ companyId, onClose, onSaved }: { companyId: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError("");
    try {
      await api(`/platform/companies/${companyId}/invoices`, { method: "POST", idempotencyKey: idempotencyKey("platform-invoice", companyId), body: JSON.stringify({ periodStart, periodEnd, issueDate, notes: notes.trim() || null, adjustments: adjustments.filter((item) => item.description.trim() && Number(item.amount) !== 0) }) });
      await onSaved();
...
function formatMoney(value: string, currency: string) {
  return new Intl.NumberFormat(activeIntlLocale(), { style: "currency", currency, currencyDisplay: "code", minimumFractionDigits: 2, maximumFractionDigits: 4 }).format(Number(value));
}

...
function RoleForm({ role, permissions, onClose, onSaved }: { role: Role | null; permissions: Permission[]; onClose: () => void; onSaved: () => void | Promise<void> }) {
  const { permissionSet } = useAuthorization();
  const [nameAr, setNameAr] = useState(role?.nameAr ?? ""), [nameEn, setNameEn] = useState(role?.nameEn ?? ""), [selected, setSelected] = useState(role?.permissionIds ?? []), [error, setError] = useState(""), [saving, setSaving] = useState(false);

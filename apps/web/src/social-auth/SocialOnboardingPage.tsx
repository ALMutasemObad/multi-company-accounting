        ...
        <div className="registration-card">
          ...
          {options && !completed && (
            <form className="registration-form" onSubmit={submit}>
              ...
              <label><span>{t('registration.interfaceLanguage')}</span><select value={locale} onChange={(event) => setLocale(resolveLocale(event.target.value))} disabled={action.busy}>{supportedLocales.filter((item) => options.locales.includes(item)).map((item) => <option key={item} value={item}>{localeDetails[item].nativeName}</option>)}</select></label>

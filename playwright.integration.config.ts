...
const internallyResponsiveSpecs = [
  '**/bank-reconciliation-practical-maturity.spec.ts',
  '**/dashboard-practical-maturity.spec.ts',
];

export default defineConfig({
  testDir: './tests',
  testMatch: ['visual/**/*.spec.ts', 'track-b/**/*.spec.ts', 'track-d/**/*.spec.ts'],
  fullyParallel: false, workers: process.env.CI ? 2 : 1, retries: 0, timeout: 120_000,
  ...
  projects: [
    ...
    { name: 'tablet-768', testIgnore: ['**/track-b/**', '**/track-c-*.spec.ts', '**/track-d/**', ...internallyResponsiveSpecs], use: { viewport: { width: 768, height: 1024 } } },
    { name: 'desktop-1440', testIgnore: internallyResponsiveSpecs, use: { viewport: { width: 1440, height: 900 } } },
    { name: 'wide-1920', testIgnore: ['**/track-b/**', '**/track-c-*.spec.ts', '**/track-d/**', ...internallyResponsiveSpecs], use: { viewport: { width: 1920, height: 1080 } } },
  ],
  webServer: [
